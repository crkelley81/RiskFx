...
public class MapEditor extends FxmlView {
	...
	private final FileDialogs<Node, FileChooser> fileDialogs;
	...
	@Inject
	public MapEditor(
			final UiContext<Node> context, 
			final FileDialogs<Node, FileChooser> fileDialogs,
			final MapEditorModel model, 
			final MapIO io) {
		this.context = Objects.requireNonNull(context);
		this.fileDialogs = Objects.requireNonNull(fileDialogs);
...
//			menu.show(navIconBtn, 0, 0);
	...
	public final void openMap() {
		fileDialogs.showOpenFileDialog(this, c -> {
	...
	public void chooseBackgroundImage() {
		fileDialogs.showOpenFileDialog(this, this::openImage)
	...
	public void chooseReliefImage() {
		fileDialogs.showOpenFileDialog(this, this::openImage).flatMap(f -> Mono.fromCallable(() -> {
			return f.toURI().toURL();
		})).subscribe(u -> {
