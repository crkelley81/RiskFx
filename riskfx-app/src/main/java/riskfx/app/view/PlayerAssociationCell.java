...
import riskfx.engine.PlayerAssociation.Type;

public class PlayerAssociationCell extends ListCell<PlayerAssociation> {

	public static Callback<ListView<PlayerAssociation>, ListCell<PlayerAssociation>> factory(final Stream<PlayerAssociation.Type> types) {
		final Collection<PlayerAssociation.Type> t = types.collect(Collectors.toUnmodifiableList());
		return lv -> new PlayerAssociationCell(t);
	}

	private final Region colorRegion = new Region();
	private final Label nameField = new Label();
	private final ComboBox<PlayerAssociation.Type> typeCombo = new ComboBox<>();
	
	private final HBox hbox = new HBox(colorRegion, nameField, typeCombo);
	
	private Runnable clear = () -> {};
	
	private PlayerAssociationCell(final Collection<Type> t) {
		super();
